...
CLI harness for **Apple Pages** — build, edit, and export Pages documents from the command line or via AI agents.
...
```bash
cd /Users/marcelrgberger/pages/agent-harness
pip install -e .
```
...
cli-anything-pages document new --template "Blank"

# Open an existing document
cli-anything-pages document open ~/Documents/report.pages

# Add text
cli-anything-pages --document "Report" text add "Hello, World!"

# Add a table
cli-anything-pages --document "Report" table add --rows 5 --cols 3

# Export as PDF
cli-anything-pages --document "Report" export pdf ~/Desktop/report.pdf

# Export as Word
cli-anything-pages --document "Report" export word ~/Desktop/report.docx

# List templates
cli-anything-pages template list
```
...
This CLI uses Apple Pages' built-in AppleScript support to control the application programmatically. Pages must be running (the CLI launches it automatically if needed). All operations are performed by the real Pages application — the CLI is an interface to Pages, not a replacement.
...
CLI_ANYTHING_FORCE_INSTALLED=1 python3 -m pytest cli_anything/pages/tests/ -v -s
```
